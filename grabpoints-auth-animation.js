(function(){
  ...
  function inject(){
    ...
    s.textContent=`
      ...
      .gz-auth-visual{position:absolute;inset:0 auto 0 0;width:50%;z-index:3;padding:42px;background:linear-gradient(145deg,#111 0%,#25186f 48%,#5b25d9 100%);color:#fff;display:flex;flex-direction:column;justify-content:center;overflow:hidden;transition:transform .72s cubic-bezier(.77,0,.18,1)}
      ...
      .gz-auth-shell.is-register .gz-auth-form{grid-column:1;transform:translateX(0)}
      .gz-auth-shell.is-register .gz-auth-stage{grid-template-columns:1fr 1fr}
      ...
      @media(max-width:720px){
        ...
        .gz-auth-visual{position:relative;inset:auto;width:100%;min-height:220px;padding:28px 25px;order:0;transform:none!important}
        .gz-auth-form{order:1;padding:30px 22px 26px;transform:none!important}
        ...
        .gz-auth-shell.is-register .gz-auth-visual{left:auto}
      }
    `;
  ...
  function build(body){
    ...
    const isRegister=!!document.getElementById('grRegister');
    const original=[...body.children];
    if(!original.length) return;
    body.dataset.gzAuthAnimated='1';
    body.classList.add('gz-auth-shell');
    if(isRegister) body.classList.add('is-register');
    ...
    const switcher=document.createElement('button'); switcher.type='button'; switcher.className='gz-auth-switch';
    switcher.textContent=isRegister?'Already have an account? Login':'New to GrabPoints? Join now';
    switcher.onclick=function(){const b2=isRegister?document.getElementById('grToLogin'):document.getElementById('grToRegister'); if(b2)b2.click()};
    visual.appendChild(switcher);
    const form=document.createElement('section'); form.className='gz-auth-form';
    ...
    try{const existing=document.getElementById('gpBrandLogo'); if(existing&&existing.src)logo.src=existing.src}catch(e){}
    setTimeout(()=>form.classList.add('is-switching'),20);
  }
  function watch(){
    inject();
    const body=document.getElementById('gzRewardsBody');
    if(!body) return;
    if(body.dataset.gzAuthObserver==='1') return;
    body.dataset.gzAuthObserver='1';
    const obs=new MutationObserver(()=>{
      if(body.dataset.gzAuthAnimated==='1') return;
      if(document.getElementById('grLogin')||document.getElementById('grRegister')) build(body);
    });
  ...
  function boot(){
    inject();
    let tries=0; const timer=setInterval(()=>{watch();if(++tries>30)clearInterval(timer)},250);
  }
