...
async function ensureSchema(env){
 ...
 if(!schemaPromise){
  schemaPromise=(async()=>{for(const sql of D1_SCHEMA)await env.DB.prepare(sql).run();
...
async function supabaseUser(req,env){
  ...
  if(!base||!key||!a.startsWith("Bearer "))return null;
  try{
    const r=await fetch(base+"/auth/v1/user",{headers:{apikey:key,Authorization:a}});
    if(!r.ok)return null;
...
async function gmailToken(env){const body=new URLSearchParams({client_id:env.GOOGLE_CLIENT_ID||"",client_secret:env.GOOGLE_CLIENT_SECRET||"",refresh_token:env.GOOGLE_REFRESH_TOKEN||"",grant_type:"refresh_token"}),r=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body}),d=await r.json().catch(()=>({}));if(!r.ok||!d.access_token)throw new Error("Google OAuth token failed: "+(d.error_description||d.error||"unknown error"));return d.access_token}
function base64Bytes(bytes){let out="";for(let i=0;i<bytes.length;i+=0x8000)out+=String.fromCharCode(...bytes.subarray(i,i+0x8000));return btoa(out)}
function base64UrlBytes(bytes){return base64Bytes(bytes).replace(/\+/g,"-").replace(/\//g,"_").replace(/=/g,"")}
async function supabaseOrderLookup(req,env,num){
 const base=String(env.SUPABASE_URL||"").replace(/\/$/,"");
 const key=String(env.SUPABASE_ANON_KEY||"");
 const auth=req.headers.get("Authorization")||"";
 if(!base||!key)throw new Error("Supabase email data service is not configured.");
 if(!auth.startsWith("Bearer "))return null;
 const r=await fetch(base+"/rest/v1/orders?order_number=eq."+encodeURIComponent(num)+"&select=*",{headers:{apikey:key,Authorization:auth,Accept:"application/json"}});
 if(!r.ok)throw new Error("Supabase order lookup failed ("+r.status+").");
 const rows=await r.json().catch(()=>[]);
 return Array.isArray(rows)?(rows[0]||null):null;
}
async function supabaseOrderItems(req,env,orderId){
 const base=String(env.SUPABASE_URL||"").replace(/\/$/,"");
 const key=String(env.SUPABASE_ANON_KEY||"");
 const auth=req.headers.get("Authorization")||"";
 if(!base||!key||!auth.startsWith("Bearer "))return [];
 const r=await fetch(base+"/rest/v1/order_items?order_id=eq."+encodeURIComponent(orderId)+"&select=*&order=id.asc",{headers:{apikey:key,Authorization:auth,Accept:"application/json"}});
 if(!r.ok)throw new Error("Supabase order-items lookup failed ("+r.status+").");
 const rows=await r.json().catch(()=>[]);
