...
function inject(){
 ...
 .gz-orders-wrap{overflow:auto}.gz-orders-table{width:100%;border-collapse:collapse;min-width:720px}.gz-orders-table th,.gz-orders-table td{padding:13px 10px;border-bottom:1px solid #eee;text-align:left;font-size:12px}.gz-orders-table th{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#777}.gz-order-link{border:0;background:none;padding:0;font:inherit;font-weight:900;cursor:pointer}.gz-status{display:inline-flex;padding:5px 9px;border-radius:999px;background:#eef1ef;font-weight:800;font-size:10px}.gz-status.cancelled{background:#fff0ef;color:#a00}.gz-status.delivered{background:#eaf8f0;color:#176b2c}.gz-empty-orders{text-align:center;padding:30px;color:#777}
...
function renderOrders(){
 ...
 const list=orders.filter(o=>(!q||`${o.order_number} ${o.customer_name} ${o.phone} ${o.email}`.toLowerCase().includes(q))&&(!st||o.status===st));
 if(!list.length){panel.innerHTML='<div class="gz-empty-orders">No orders found.</div>';return}
 panel.innerHTML=`<div class="gz-orders-wrap"><table class="gz-orders-table"><thead><tr><th>Order</th><th>Customer</th><th>Phone</th><th>Total</th><th>Status</th><th>Created</th></tr></thead><tbody>${list.map(o=>`<tr><td><button class="gz-order-link" data-order="${o.id}">${esc(o.order_number)}</button></td><td>${esc(o.customer_name)}</td><td>${esc(o.phone)}</td><td><b>${money(o.total)}</b></td><td><span class="gz-status ${o.status.toLowerCase()}">${esc(o.status)}</span></td><td>${new Date(o.created_at).toLocaleString()}</td></tr>`).join('')}</tbody></table></div>`;
...
async function openEditor(id){
 ...
 const {data:items,error}=await sb.from('order_items').select('*').eq('order_id',id).order('id');if(error){alert(error.message);return}
 current={...base,items:items||[]};
 $('gzOrderEditorTitle').textContent=current.order_number; $('gzOrderEditorSub').textContent=`Placed ${new Date(current.created_at).toLocaleString()} · Last updated ${new Date(current.updated_at).toLocaleString()}`;
...
async function saveEditor(){
 if(!current)return; const oldStatus=current.status;
 const payload={customer_name:$('oeName').value.trim(),phone:$('oePhone').value.trim(),email:$('oeEmail').value.trim(),division:$('oeDivision').value.trim(),district:$('oeDistrict').value.trim(),upazila:$('oeUpazila').value.trim(),address:$('oeAddress').value.trim(),referral_code:$('oeReferral').value.trim()||null,payment_method:$('oePayment').value.trim()||'Cash on Delivery',shipping_charge:Number($('oeShipping').value||0),status:$('oeStatus').value,admin_note:$('oeNote').value.trim()||null};
 const items=[...document.querySelectorAll('.gz-item-edit')].map((row,i)=>({product_id:current.items[i]?.product_id||null,product_name:row.querySelector('.it-name').value.trim(),quantity:Math.max(1,Number(row.querySelector('.it-qty').value||1)),unit_price:Math.max(0,Number(row.querySelector('.it-price').value||0)),image_url:row.querySelector('.it-image').value.trim()})).filter(x=>x.product_name);
 ...
 try{
  ...
  if(payload.status!==oldStatus){try{await fetch('/api/send-order-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({type:'status_updated',orderNumber:current.order_number,order:{...payload,items}})});}catch(e){console.warn(e)}}
  $('gzOrderEditorMsg').textContent='✓ Order updated successfully.';await loadOrders();setTimeout(closeEditor,500);
