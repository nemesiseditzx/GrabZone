...
const pc=Number((await one(e,'SELECT COUNT(*) n FROM products WHERE vendor_id=?',[v.id]))?.n||0),oc=Number((await one(e,'SELECT COUNT(*) n FROM vendor_orders WHERE vendor_id=?',[v.id]))?.n||0);
...
async function track(req,e,id){const o=await one(e,'SELECT * FROM orders WHERE upper(public_tracking_id)=upper(?) OR upper(order_number)=upper(?)',[id,id]);if(!o)return json({error:'Order not found'},404);const vs=(await q(e,'SELECT vo.*,v.brand_name,v.slug,v.logo_url FROM vendor_orders vo JOIN vendors v ON v.id=vo.vendor_id WHERE vo.order_id=?',[o.id])).results||[];for(const v of vs){v.items=(await q(e,'SELECT voi.*,oi.product_name,oi.image_url FROM vendor_order_items voi JOIN order_items oi ON oi.id=voi.order_item_id WHERE voi.vendor_order_id=?',[v.id])).results||[];v.shipments=(await q(e,'SELECT * FROM shipments WHERE order_id=? AND vendor_id=? ORDER BY created_at',[o.id,v.vendor_id])).results||[]}const statuses=vs.map(v=>String(v.status||'New'));let canonical=o.status||'New';if(statuses.length){if(statuses.every(s=>s==='Delivered'))canonical='Delivered';else if(statuses.every(s=>s==='Cancelled'))canonical='Cancelled';else if(statuses.every(s=>s==='Shipped'))canonical='Shipped';else if(statuses.every(s=>s==='Processing'))canonical='Processing';else if(statuses.every(s=>s==='Confirmed'))canonical='Confirmed';else if(statuses.every(s=>s==='Contacting'))canonical='Contacting';else if(statuses.some(s=>s==='Shipped'))canonical='Shipped';else if(statuses.some(s=>s==='Processing'))canonical='Processing';else if(statuses.some(s=>s==='Confirmed'))canonical='Confirmed';else if(statuses.some(s=>s==='Contacting'))canonical='Contacting';}return json({order:{order_number:o.order_number,tracking_id:o.public_tracking_id,status:canonical,created_at:o.created_at,updated_at:o.updated_at,subtotal:o.subtotal,shipping_charge:o.shipping_charge,total:o.total},vendors:vs})}
