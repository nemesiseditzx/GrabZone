...
if(p==='/api/vendor/admin/overview'){
 ...
 const totals=await one(e,"SELECT COALESCE(SUM(subtotal),0) sales,COUNT(DISTINCT order_id) orders,COALESCE(SUM(commission_amount),0) commission,COALESCE(SUM(vendor_earnings),0) earnings FROM vendor_orders")||{sales:0,orders:0,commission:0,earnings:0};
 const vendors=(await q(e,"SELECT v.id,v.slug,v.brand_name,v.business_name,v.status,COALESCE((SELECT COUNT(*) FROM vendor_products vp WHERE vp.vendor_id=v.id),0) products,COALESCE((SELECT COUNT(DISTINCT vo.order_id) FROM vendor_orders vo WHERE vo.vendor_id=v.id),0) orders,COALESCE((SELECT SUM(vo.subtotal) FROM vendor_orders vo WHERE vo.vendor_id=v.id),0) sales,COALESCE((SELECT SUM(vo.commission_amount) FROM vendor_orders vo WHERE vo.vendor_id=v.id),0) commission,COALESCE((SELECT SUM(vo.vendor_earnings) FROM vendor_orders vo WHERE vo.vendor_id=v.id),0) earnings FROM vendors v ORDER BY v.featured DESC,v.brand_name")).results||[];
