...
const VENDOR_PRODUCT_EDITOR=`<script data-gz-vendor-product-editor-loader>(()=>{if(window.__gzVendorProductEditorLoader)return;window.__gzVendorProductEditorLoader=1;const load=async()=>{if(window.__gzVendorProductEditor)return;try{const r=await fetch('/vendor-control-product-editor.js?v=20260918-v3',{cache:'no-store',credentials:'same-origin'});if(!r.ok)throw new Error('Product editor asset failed: '+r.status);const code=await r.text();const s=document.createElement('script');s.textContent=code;document.head.appendChild(s);}catch(e){console.error('[GrabZone] vendor product editor failed',e);}};if(document.readyState==='complete')load();else window.addEventListener('load',load,{once:true});})();</script>`;
