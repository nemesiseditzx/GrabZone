...
function builder(table){
 ...
 const a={
  ...
  eq(c,v){s.filters.push({column:c,op:'eq',value:v});return a},neq(c,v){s.filters.push({column:c,op:'neq',value:v});return a},gt(c,v){s.filters.push({column:c,op:'gt',value:v});return a},gte(c,v){s.filters.push({column:c,op:'gte',value:v});return a},lt(c,v){s.filters.push({column:c,op:'lt',value:v});return a},lte(c,v){s.filters.push({column:c,op:'lte',value:v});return a},is(c,v){s.filters.push({column:c,op:'is',value:v});return a},in(c,v){s.filters.push({column:c,op:'in',value:v});return a},order(c,o={}){s.orders.push({column:c,ascending:o?.ascending!==false});return a},limit(n){s.limit=Number(n);return a},maybeSingle(){s.single='maybe';return execute()},single(){s.single='single';return execute()},
  insert(v,o={}){s.action='insert';if(table==='product_images'){const now=new Date().toISOString();s.values=(Array.isArray(v)?v:[v]).map(row=>row?.created_at?row:{...row,created_at:now});}else s.values=v;s.returning=!!o?.returning;return a},
  upsert(v,o={}){s.action='upsert';s.values=v;s.returning=true;s.conflict=o?.onConflict||null;return a},update(v){s.action='update';s.values=v;return a},delete(){s.action='delete';return a},then(res,rej){return execute().then(res,rej)}
 };
...
try{const pagePath=String(location.pathname||'').replace(/\/+$/,'').toLowerCase();const isGrabPointsPage=pagePath==='/grabpoints'||pagePath==='/grabpoints.html'||pagePath.endsWith('/grabpoints')||pagePath.endsWith('/grabpoints.html');if(isGrabPointsPage){const s=document.createElement('script');s.src='grabpoints-auth-animation.js?v=20260908ui5';s.async=false;document.head.appendChild(s);const rp=document.createElement('script');rp.src='grabpoints-redeem-polish.js?v=20260908polish3';rp.async=false;document.head.appendChild(rp);const rm=document.createElement('script');rm.src='grabpoints-redeem-motion.css.js?v=20260908motion1';rm.async=false;document.head.appendChild(rm);const rs=document.createElement('script');rs.src='grabpoints-redeem-sound.js?v=20260908sound1';rs.async=false;document.head.appendChild(rs);}}
catch{}
})();