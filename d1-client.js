...
window.grabzoneD1={from:builder,rpc:(fn,args={})=>request({type:'rpc',fn,args}),auth:{
 async signInWithPassword({email,password}){try{const r=await fetch('/api/admin-auth',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',cache:'no-store',headers:{'Content-Type':'application/json','Cache-Control':'no-cache'},body:JSON.stringify({action:'login',email,password})}),b=await r.json().catch(()=>({}));if(!r.ok||!b.ok)return{data:{user:null,session:null},error:{message:b.error||'Invalid email or password.',status:r.status}};setToken(b.session_token||'');return{data:{user:b.user||null,session:b.session_token?{access_token:b.session_token,user:b.user||null,expires_at:b.expires_at}:null},error:null}}catch(e){return{data:{user:null,session:null},error:{message:e.message||'Authentication failed.'}}}},
 async getSession(){
 try{
  const existing=getToken();
  const r=await fetch('/api/admin-auth',{method:'GET',credentials:'include',cache:'no-store',headers:{'Cache-Control':'no-cache'}});
 ...
 async signOut(){try{await fetch('/api/admin-auth',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',cache:'no-store',headers:{'Content-Type':'application/json','Cache-Control':'no-cache'},body:JSON.stringify({action:'logout'})})}finally{setToken('')}return{error:null}}
}};
