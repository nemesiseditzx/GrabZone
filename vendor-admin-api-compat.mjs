...
async function syncShipmentStatus(e,orderId,vendorOrderId,status){
  await e.DB.prepare('UPDATE vendor_orders SET status=?,updated_at=? WHERE id=?').bind(status,now(),vendorOrderId).run();
  const rows=await all(e,'SELECT status FROM vendor_orders WHERE order_id=?',[orderId]),ss=rows.map(x=>String(x.status||''));
  let parent='New';
  if(!ss.length)parent='New';
  else if(ss.every(x=>x==='Delivered'))parent='Delivered';
  else if(ss.every(x=>x==='Cancelled'))parent='Cancelled';
  else if(ss.every(x=>x==='Shipped'))parent='Shipped';
  else if(ss.every(x=>x==='Processing'))parent='Processing';
  else if(ss.every(x=>x==='Confirmed'))parent='Confirmed';
  else if(ss.every(x=>x==='Contacting'))parent='Contacting';
  else if(ss.some(x=>x==='Shipped'))parent='Shipped';
  else if(ss.some(x=>x==='Processing'))parent='Processing';
  else if(ss.some(x=>x==='Confirmed'))parent='Confirmed';
  else if(ss.some(x=>x==='Contacting'))parent='Contacting';
  await e.DB.prepare('UPDATE orders SET status=?,updated_at=? WHERE id=?').bind(parent,now(),orderId).run();
  return parent;
}
...
async function shipmentControl(req,e){
  ...
  if(req.method==='GET')return json({shipments:await all(e,'SELECT * FROM shipments WHERE order_id=? AND vendor_id=? ORDER BY created_at DESC',[vo.order_id,vo.vendor_id])});
  const courier=clean(b.courier||b.courier_name,120);
  const tracking=clean(b.tracking_id||b.tracking_number,160);
  const trackingUrl=clean(b.tracking_url,1000);
  const st=clean(b.status||'Processing',50);
  const allowedShipmentStatus=['New','Contacting','Confirmed','Processing','Shipped','Delivered','Cancelled'];
  if(!allowedShipmentStatus.includes(st))return json({error:'Invalid shipment status.'},400);
  if(trackingUrl&&!/^https?:\/\//i.test(trackingUrl))return json({error:'Tracking URL must use http:// or https://.'},400);
  if(req.method==='POST'){
    ...
    const parent=await syncShipmentStatus(e,vo.order_id,vo.id,st);
    return json({ok:true,shipment:await one(e,'SELECT * FROM shipments WHERE id=?',[id]),parent_status:parent},201);
  }
  ...
  await e.DB.prepare('UPDATE shipments SET courier=?,tracking_id=?,tracking_url=?,status=?,updated_at=? WHERE id=? AND order_id=? AND vendor_id=?').bind(courier,tracking,trackingUrl,st,now(),sid,vo.order_id,vo.vendor_id).run();
  const parent=await syncShipmentStatus(e,vo.order_id,vo.id,st);
  return json({ok:true,shipment:await one(e,'SELECT * FROM shipments WHERE id=?',[sid]),parent_status:parent});
}

export async function handleVendorAdminApi(req,e){try{return await orderData(req,e)||await shipmentControl(req,e)||await overview(req,e)||await orderStatus(req,e)||await products(req,e)||await categories(req,e)||await sections(req,e)}catch(err){console.error('Vendor admin API error',err);return json({error:'Internal server error.'},500)}}