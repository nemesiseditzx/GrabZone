import marketplace from './vendor-product-media-router.mjs';
...
if((p==='/checkout.html'||p==='/checkout')&&req.method==='GET')return marketplace.fetch(req,e);
...
if(p==='/api/vendor/admin/order-status'&&req.method==='PATCH'){const a=await admin(req,e);if(!a)return json({error:'Unauthorized'},401);let b={};try{b=await req.json()}catch{return json({error:'Invalid JSON'},400)}const vo=await one(e,'SELECT vo.id,vo.order_id,o.order_number FROM vendor_orders vo JOIN orders o ON o.id=vo.order_id WHERE vo.id=?',[clean(b.vendor_order_id,100)]);const r=await marketplace.fetch(req,e);if(r.ok&&vo?.order_number)await notify(e,vo.order_number);return r}
