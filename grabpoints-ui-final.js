...
/* Public GrabPoints page: keep the entire information layer on the GrabZone orange/black/white system. */
:root{--gz-orange:#ff6b00;--gz-orange-soft:#fff5ec;--gz-ink:#171717;--gz-muted:#6f6f73;--gz-line:#e7e1dc}
.wrap .eyebrow{color:#e85d04!important}
.rewards-shell{background:linear-gradient(135deg,#fffaf5 0%,#fff 55%,#fff8f2 100%)!important;border-color:#eadfd6!important}
.membership-cta{background:linear-gradient(135deg,#fff,#fff8f2)!important;border-color:#eadfd6!important}
.membership-cta .btn{background:#171717!important}
.tier.tier-member,.tier.tier-silver,.tier.tier-gold,.tier.tier-platinum,.tier.tier-diamond{background:linear-gradient(135deg,#fffaf5,#fff)!important;border-color:#eadfd6!important}
.tier.tier-member b,.tier.tier-silver b,.tier.tier-gold b,.tier.tier-platinum b,.tier.tier-diamond b{color:#d9570b!important}
.note{background:#fff8f2!important;border-color:#f1dfd1!important}

/* Mobile history remains discoverable instead of disappearing from the layout. */
@media(max-width:760px){.gz-stat-grid{grid-template-columns:1fr 1fr!important}.gz-redeem-premium{padding:15px!important}.gz-rf-card{max-height:96vh!important}.gz-rf-digit{width:50px!important;height:50px!important}.gz-history-side{display:block!important}.gz-dash-layout{grid-template-columns:1fr!important}}
...
function fixPublicCopy(){
 const root=document.querySelector('.rewards-shell');
 if(root){
  root.querySelectorAll('.en,.bn').forEach(el=>{
   const t=(el.textContent||'').trim();
   if(/Earn GP from eligible orders/i.test(t)) el.textContent='Earn GP from every eligible member order, based on your membership cashback tier.';
   if(/যোগ্য order থেকে GP আয় করুন/.test(t)) el.textContent='প্রতিটি eligible member order থেকে আপনার membership cashback tier অনুযায়ী GP earn করুন।';
  });
 }
 const sectionHeads=document.querySelectorAll('.section-head p');
 sectionHeads.forEach(p=>{
  const t=(p.textContent||'').trim();
  if(/optional loyalty system/i.test(t)) p.textContent="GrabPoints (GP) is GrabZone's member rewards system. Members earn GP cashback on eligible purchases according to their membership tier.";
  if(/optional loyalty system/i.test(t)===false && /optional loyalty system/i.test(t)) return;
 });
 const cards=document.querySelectorAll('.card p');
 cards.forEach(p=>{
  const t=(p.textContent||'').trim();
  if(/Eligible completed orders can add GP/i.test(t)) p.textContent='Members earn GP cashback on eligible purchases. Your membership tier determines the cashback rate.';
 });
 const cashback={member:'1%',silver:'2%',gold:'3%',platinum:'5%'};
 document.querySelectorAll('#gpLiveTiers .tier').forEach(card=>{
  const key=[...card.classList].find(x=>x.indexOf('tier-')===0);
  if(!key)return;
  const rate=cashback[key.replace('tier-','').toLowerCase()];
  if(!rate||card.querySelector('.gz-cashback-rate'))return;
  const b=document.createElement('div');b.className='gz-cashback-rate';b.textContent='GP cashback: '+rate;b.style.cssText='margin-top:10px;font-size:12px;font-weight:900;color:#d9570b';card.appendChild(b);
 });
}
...
function boot(){css();fixPublicCopy();stabilize();const mo=new MutationObserver(()=>fixPublicCopy());mo.observe(document.body,{childList:true,subtree:true});setTimeout(()=>mo.disconnect(),12000);}
