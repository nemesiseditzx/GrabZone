...
async function customerVariations(){
 ...
 const doAdd=()=>{const x=item();if(!x||!variationAvailable(current)){window.gzNotify('Please select an available variation first.');return}x.quantity=cvQty;let cart=[];try{cart=JSON.parse(localStorage.getItem('grabzone_cart_v2')||'[]');if(!Array.isArray(cart))cart=[]}catch{}const same=cart.find(y=>String(y.product_id)===String(x.product_id)&&String(y.variation_id)===String(x.variation_id));if(same)same.quantity=Number(same.quantity||0)+cvQty;else cart.push(x);localStorage.setItem('grabzone_cart_v2',JSON.stringify(cart));window.dispatchEvent(new Event('grabzone-cart-updated'));addBtn.textContent='Added ✓';setTimeout(()=>addBtn.textContent='Add to Cart',900)};
 const doBuy=()=>{const x=item();if(!x||!variationAvailable(current)){window.gzNotify('Please select an available variation first.');return}x.quantity=cvQty;localStorage.setItem('grabzone_buy_now_v2',JSON.stringify([x]));location.href='/checkout'};
