...
async function customerVariations(){
 ...
 function updatePrice(v){
   if(!v)return;
   const price=v.sale_price!=null?Number(v.sale_price):Number(v.regular_price||0);
   detail.querySelectorAll('#productPrice,.product-price,.price,.detail-price,[data-product-price]').forEach(el=>{el.textContent=money(price)});
   detail.querySelectorAll('[data-sale-price]').forEach(el=>{el.textContent=v.sale_price!=null?money(v.sale_price):''});
 ...
 function update(){
   ...
   const price=current.sale_price!=null?current.sale_price:current.regular_price;
   ...
   qty=Math.max(min,qty);if(max)qty=Math.min(max,qty);if(stock&&qty>stock)qty=stock;
 ...
 area.querySelectorAll('button[data-option]').forEach(b=>b.onclick=()=>{selected[b.dataset.option]=b.dataset.value;qty=1;update()});
 wrap.querySelector('#gzCVDec').onclick=()=>{if(current)qty=Math.max(Number(current.min_qty||1),qty-1);update()};
 wrap.querySelector('#gzCVInc').onclick=()=>{if(current){const max=current.max_qty?Number(current.max_qty):Infinity,stock=Number(current.stock||0);if(qty<max&&qty<stock)qty++}update()};
 function item(){if(!current)return null;const price=current.sale_price!=null?Number(current.sale_price):Number(current.regular_price||0);return{product_id:pid,variation_id:current.id,variation_options:current.options||{},quantity:qty,unit_price:price,price,name:data.product?.name||'Product',image_url:imageFor(current)||data.product?.image_url||'',sku:current.sku||''}}
 function addCart(){const x=item();if(!x||Number(current.stock||0)<qty)return false;let cart=[];try{cart=JSON.parse(localStorage.getItem('grabzone_cart_v2')||'[]');if(!Array.isArray(cart))cart=[]}catch{}const existing=cart.find(y=>y.product_id===x.product_id&&y.variation_id===x.variation_id);if(existing){const max=current.max_qty?Number(current.max_qty):Infinity;existing.quantity=Math.min(max,Number(existing.quantity||0)+qty)}else cart.push(x);localStorage.setItem('grabzone_cart_v2',JSON.stringify(cart));return true}
 function buyNow(){const x=item();if(!x||Number(current.stock||0)<qty)return false;localStorage.setItem('grabzone_buy_now_v2',JSON.stringify([x]));location.href='checkout.html';return true}
 function intercept(e){
   ...
   if(Number(current.stock||0)<qty){e.preventDefault();e.stopImmediatePropagation();alert('This variation is out of stock.');return}
