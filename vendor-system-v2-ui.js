...
async function customerVariations(){
 ...
 wrap.innerHTML='<div class="gz-cv-title">Choose your options</div><div id="gzCVOptions"></div><div id="gzCVState" class="gz-cv-state warn">Select all options</div><div class="gz-cv-qty"><button type="button" data-gz-qty-minus>−</button><span id="gzCVQty">1</span><button type="button" data-gz-qty-plus>+</button></div><div class="gz-cv-actions"><button type="button" class="btn btn-dark" data-gz-add>Add to Cart</button><button type="button" class="btn btn-dark" data-gz-buy>Buy Now</button></div><div class="gz-cv-actions-note" id="gzCVQtyNote"></div>';
 ...
 const colorMap={black:'#111111',white:'#ffffff',grey:'#808080',gray:'#808080',brown:'#8b4513',beige:'#f5f5dc',tan:'#d2b48c',cream:'#fffdd0',khaki:'#c3b091','off white':'#f8f8f2','charcoal black':'#222222','light grey':'#d3d3d3',camel:'#c19a6b','charcoal grey':'#4a4a4a','dark grey':'#555555',bronze:'#cd7f32','mauve brown':'#915f6d','mustard grey':'#a39a62','pale grey':'#e5e5e5','peach beige':'#f5c7a9','rust brown':'#a0522d',red:'#e53935',maroon:'#800000','deep red':'#b11226',crimson:'#dc143c',rust:'#b7410e',wine:'#722f37','wine maroon':'#5a1f2b',pink:'#ff69b4',magenta:'#d100c9',rose:'#e8a0a8',peach:'#ffcba4','light pink':'#ffb6c1','mauve pink':'#d8a0b8','coral pink':'#ff7f8a','dusty pink':'#d8a0a6','dusty rose':'#c98f96','magenta pink':'#e754b8','raspberry pink':'#e30b5c',purple:'#800080',lavender:'#b57edc',mauve:'#e0b0ff',plum:'#8e4585','deep purple':'#4b0082','light purple':'#b39ddb',violet:'#7f00ff',blue:'#1565c0','sky blue':'#87ceeb','navy blue':'#000080',navy:'#001f3f','light blue':'#add8e6',denim:'#1560bd','royal blue':'#4169e1','steel blue':'#4682b4','dark navy':'#000033','light blue denim':'#6f9fcf','dark blue denim':'#234f7d','teal blue':'#367588','cornflower blue':'#6495ed','dark teal blue':'#014d4e','denim blue':'#3f6fa0','light denim blue':'#7aa5d2','pale blue':'#c7e6f5','slate blue':'#6a5acd',teal:'#008080',aqua:'#00ffff','mint aqua':'#a8e6cf','teal green':'#008f7a','aqua green':'#00bfa5',green:'#2e7d32','olive':'#808000','olive green':'#6b8e23','sage green':'#9caf88','dark green':'#006400',mint:'#98ff98','mint green':'#98fb98','sea green':'#2e8b57','light green':'#90ee90','pale sage':'#d7e4c0','lime green':'#32cd32','pale mint':'#d8f3dc',sage:'#b2ac88','mint pista':'#bde7b5',yellow:'#ffd600',orange:'#ff8c00',mustard:'#d4a017',gold:'#d4af37','mustard yellow':'#e1ad01','pale yellow':'#fff59d',multi:'linear-gradient(135deg,#ef4444 0 16%,#f59e0b 16% 32%,#facc15 32% 48%,#22c55e 48% 64%,#3b82f6 64% 80%,#a855f7 80%)'};
 ...
 function renderOptions(){
   ...
   names.forEach(name=>{
     ...
     row.innerHTML='<label>'+esc(name)+'</label><div class="gz-cv-values">'+vals.map(value=>{
       const c=isColor?colorMap[String(value).trim().toLowerCase()]||'#d9d9d9':null;
 ...
 const doAdd=()=>{const x=item();if(!x||!variationAvailable(current)){alert('Please select an available variation first.');return}x.quantity=cvQty;let cart=[];try{cart=JSON.parse(localStorage.getItem('grabzone_cart_v2')||'[]');if(!Array.isArray(cart))cart=[]}catch{}const same=cart.find(y=>String(y.product_id)===String(x.product_id)&&String(y.variation_id)===String(x.variation_id));if(same)same.quantity=Number(same.quantity||0)+cvQty;else cart.push(x);localStorage.setItem('grabzone_cart_v2',JSON.stringify(cart));window.dispatchEvent(new Event('grabzone-cart-updated'));addBtn.textContent='Added ✓';setTimeout(()=>addBtn.textContent='Add to Cart',900)};
 const doBuy=()=>{const x=item();if(!x||!variationAvailable(current)){alert('Please select an available variation first.');return}x.quantity=cvQty;localStorage.setItem('grabzone_buy_now_v2',JSON.stringify([x]));location.href='/checkout'};
