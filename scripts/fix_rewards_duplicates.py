...
s,n=re.subn(r'^async function forgot\(\)\{.*?^\}',lambda _m:forgot,s,count=1,flags=re.M|re.S)
...
while True:
    j=s.find(needle,i)
    if j<0: break
    pos.append(j);i=j+len(needle)
if not pos: raise SystemExit('showVerify renderer not found')
# Keep exactly one renderer; replace the first with the clean renderer and remove later copies.
first=pos[0];end=s.find('\n',first)
if end<0: end=len(s)
...
p.write_text(s)
print('Rewards PIN recovery renderer finalized')
