...
if positions:
    first=positions[0];end=s.find('\n',first);end=len(s) if end<0 else end
    s=s[:first]+show+s[end+1:]
    positions=[];i=0
    while True:
        j=s.find(needle,i)
        if j<0: break
        positions.append(j);i=j+len(needle)
    for j in reversed(positions[1:]):
        end=s.find('\n',j);end=len(s) if end<0 else end
        s=s[:j]+s[end+1:]
else:
    marker='async function verifyCode(email,code)'
    idx=s.find(marker)
    if idx<0: raise SystemExit('verifyCode function not found')
    s=s[:idx]+show+'\n'+s[idx:]
if s.count('async function forgot()')!=1 or s.count('function showVerify(email)')!=1 or 'gz-fp-clean' not in s: raise SystemExit('Rewards renderer invariant failed')
p.write_text(s)
print('Rewards PIN recovery renderer restored')
