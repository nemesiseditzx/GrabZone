const GZMP_UNIFIED_CSS=".gzmp-easy-setup{border:1px solid #ffd6b6;background:#fff8f2;border-radius:12px;padding:12px;margin:12px 0}.gzmp-easy-setup>div{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:8px;font-size:11px}.gzmp-easy-setup span{background:#fff;border-radius:9px;padding:8px}.gzmp-preset-card{border:1px solid #e1e4e7;border-radius:12px;padding:10px;margin:8px 0;background:#fff}.gzmp-preset-head{display:flex;justify-content:space-between;margin-bottom:8px;font-size:12px}.gzmp-chip-grid{display:flex;flex-wrap:wrap;gap:7px}.gzmp-chip{display:inline-flex;align-items:center;gap:5px;border:1px solid #dfe3e7;border-radius:8px;padding:7px 9px;background:#fff;font-size:10px;font-weight:800;cursor:pointer}.gzmp-chip input{accent-color:#ff6b00}.gzmp-chip:has(input:checked){border-color:#ff6b00;background:#fff3e8}.gzmp-color-search{width:100%;box-sizing:border-box;border:1px solid #ddd;border-radius:9px;padding:9px;margin-bottom:8px}.gzmp-var-image{display:grid;gap:5px;min-width:130px}.gzmp-var-image img{width:58px;height:58px;object-fit:cover;border-radius:7px;border:1px solid #ddd}.gzmp-custom-inline{display:flex;gap:7px;margin-top:8px}.gzmp-custom-inline input{flex:1;min-width:0;border:1px solid #ddd;border-radius:8px;padding:8px}.gzmp-variation-table{min-width:1050px}@media(max-width:760px){.gzmp-easy-setup>div{grid-template-columns:1fr}}"; if(typeof document!=='undefined')document.head.insertAdjacentHTML('beforeend','<style id="gzmp-unified-product-css">'+GZMP_UNIFIED_CSS+'</style>');if(typeof document!=='undefined')document.head.insertAdjacentHTML('beforeend',"<style id=\"gzmp-vendor-mirror-css\">\n.gzmp-vendor-hero{display:flex;justify-content:space-between;gap:18px;align-items:center;padding:22px;border-radius:20px;background:linear-gradient(135deg,#fff7ef,#fff);border:1px solid #f0dfd1;margin-bottom:14px}.gzmp-vendor-hero h2{margin:6px 0 4px;font-size:24px}.gzmp-vendor-hero p{margin:0;color:#777}.gzmp-store-head{display:flex;justify-content:space-between;gap:14px;align-items:center;margin-bottom:14px}.gzmp-store-head h3{font-size:22px;margin:6px 0}.gzmp-toggle-row{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:13px 0;border-top:1px solid #eee;font-weight:800;font-size:12px}.gzmp-toggle-row input{width:18px;height:18px;accent-color:#ff6b00}.gzmp-var-image-picker{width:245px}.gzmp-var-image-grid{display:flex;gap:6px;flex-wrap:wrap;max-width:245px}.gzmp-var-image-choice{position:relative;width:45px;height:45px;padding:2px;border:1px solid #dfe3e7;border-radius:7px;background:#fff;cursor:pointer}.gzmp-var-image-choice img{display:block;width:100%;height:100%;object-fit:cover;border-radius:5px}.gzmp-var-image-choice span{position:absolute;right:2px;bottom:2px;min-width:13px;height:13px;line-height:13px;text-align:center;border-radius:4px;background:rgba(0,0,0,.72);color:#fff;font-size:8px;font-weight:900}.gzmp-var-image-choice.selected{border:2px solid #ff6b00;box-shadow:0 0 0 2px rgba(255,107,0,.13)}.gzmp-var-image-choice.selected:before{content:'✓';position:absolute;z-index:2;left:2px;top:2px;width:15px;height:15px;line-height:15px;text-align:center;border-radius:50%;background:#ff6b00;color:#fff;font-size:9px;font-weight:900}.gzmp-var-image-picker small{display:block;margin-top:5px;color:#8a929a;font-size:8px}.gzmp-vpane-card{border:1px solid #e4e4df;border-radius:16px;padding:15px;background:#fff}\n@media(max-width:760px){.gzmp-vendor-hero,.gzmp-store-head{align-items:flex-start;flex-direction:column}.gzmp-var-image-picker{width:205px}.gzmp-var-image-grid{max-width:205px}}\n</style>");
(()=>{'use strict';
...
const openVendorPanel=async id=>{
 ...
 try{
  ...
  <div class="gzmp-subtabs"><button class="gzmp-subtab active" data-vsub="overview">Dashboard</button><button class="gzmp-subtab" data-vsub="products">Products</button><button class="gzmp-subtab" data-vsub="orders">Orders</button><button class="gzmp-subtab" data-vsub="store">Store</button><button class="gzmp-subtab" data-vsub="settings">Settings</button></div>
  <div id="gzmp-vpane-overview"></div><div id="gzmp-vpane-products" class="gzmp-hidden"></div><div id="gzmp-vpane-orders" class="gzmp-hidden"></div><div id="gzmp-vpane-store" class="gzmp-hidden"></div><div id="gzmp-vpane-settings" class="gzmp-hidden"></div>`);
  $('gzmpModalBody').querySelectorAll('[data-close]').forEach(b=>b.onclick=closeModal);
  document.querySelectorAll('[data-vsub]').forEach(b=>b.onclick=()=>showVendorSub(b.dataset.vsub));
  renderVendorOverview(v,d);renderVendorProducts(d.products||[]);renderVendorOrders(d.orders||[]);renderVendorStore(v);await renderVendorSections();renderVendorSettings(v);showVendorSub('overview');
...
const showVendorSub=t=>{document.querySelectorAll('[data-vsub]').forEach(b=>b.classList.toggle('active',b.dataset.vsub===t));['overview','products','orders','store','settings'].forEach(x=>$('gzmp-vpane-'+x)?.classList.toggle('gzmp-hidden',x!==t));};
const renderVendorOverview=(v,d)=>{
 const pane=$('gzmp-vpane-overview');if(!pane)return;
 const products=d.products||[],orders=d.orders||[];
 const sales=orders.reduce((n,o)=>n+Number(o.subtotal||o.order_subtotal||0),0);
 const earnings=orders.reduce((n,o)=>n+Number(o.vendor_earnings||0),0);
 const open=orders.filter(o=>!['Delivered','Cancelled'].includes(o.status)).length;
 pane.innerHTML=`<div class="gzmp-vendor-hero"><div><span class="gzmp-kicker">VENDOR DASHBOARD</span><h2>Welcome, ${val(v.brand_name||v.business_name)}</h2><p>${val(v.tagline||'Manage this vendor using the same GrabZone workflow as the vendor portal.')}</p></div><div class="gzmp-actions"><a class="gzmp-btn" href="marketplace-store.html?slug=${encodeURIComponent(v.slug)}" target="_blank">View Store ↗</a><button class="gzmp-btn primary" id="gzmpOverviewAdd">＋ Add Product</button></div></div><div class="gzmp-grid"><div class="gzmp-stat"><b>${products.length}</b><small>Products</small></div><div class="gzmp-stat"><b>${orders.length}</b><small>Orders</small></div><div class="gzmp-stat"><b>৳${sales.toLocaleString('en-BD')}</b><small>Sales</small></div><div class="gzmp-stat"><b>৳${earnings.toLocaleString('en-BD')}</b><small>Net earnings</small></div></div><div class="gzmp-card"><h3>Fulfillment</h3><p>${open} order(s) still need delivery progress updates.</p><button class="gzmp-btn" id="gzmpOverviewOrders">Open Orders →</button></div>`;
 $('gzmpOverviewAdd').onclick=()=>openProductEditor({},true);$('gzmpOverviewOrders').onclick=()=>showVendorSub('orders');
};
const renderVendorStore=(v)=>{
 const pane=$('gzmp-vpane-store');if(!pane)return;
 let social='{}',contact='{}';try{social=typeof v.social_links==='string'?v.social_links:JSON.stringify(v.social_links||{},null,2)}catch{}try{contact=typeof v.contact_info==='string'?v.contact_info:JSON.stringify(v.contact_info||{},null,2)}catch{}
 pane.innerHTML=`<div class="gzmp-store-head"><div><span class="gzmp-kicker">STOREFRONT BUILDER</span><h3>Your Store</h3><p class="gzmp-section-sub">Same store controls available in the vendor portal.</p></div><a class="gzmp-btn" href="marketplace-store.html?slug=${encodeURIComponent(v.slug)}" target="_blank">View Store ↗</a></div>
 <form id="gzmpAdminVendorStoreForm" class="gzmp-form">
  <div class="gzmp-card"><h3>Store identity</h3><div class="gzmp-form"><label>Business name<input name="business_name" value="${val(v.business_name)}"></label><label>Brand / store name<input name="brand_name" value="${val(v.brand_name)}"></label><label>Phone<input name="phone" value="${val(v.phone)}"></label><label>Accent color<input name="accent_color" type="color" value="${/^#[0-9a-f]{6}$/i.test(v.accent_color||'')?v.accent_color:'#ff6b00'}"></label><label class="full">Tagline<input name="tagline" value="${val(v.tagline)}"></label><label class="full">Description<textarea name="description" rows="5">${val(v.description)}</textarea></label></div></div>
  <div class="gzmp-card"><h3>Store branding</h3><p class="gzmp-section-sub">JPG, PNG, WEBP, GIF or AVIF · maximum 1 MB each.</p><div class="gzmp-form"><label class="full">Store logo<input name="logo_file" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif"><input name="logo_url" value="${val(v.logo_url)}" placeholder="Logo URL"></label><label class="full">Store banner<input name="banner_file" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif"><input name="banner_url" value="${val(v.banner_url)}" placeholder="Banner URL"></label></div></div>
  <div class="gzmp-card"><h3>Customer-facing content</h3><div class="gzmp-form"><label class="full">Announcement<textarea name="announcement" rows="3">${val(v.announcement)}</textarea></label><label>Contact information JSON<textarea name="contact_info" rows="6">${val(contact)}</textarea></label><label>Social links JSON<textarea name="social_links" rows="6">${val(social)}</textarea></label></div></div>
  <div class="full gzmp-actions"><button class="gzmp-btn primary">Save store changes</button><span id="gzmpStoreMsg" class="gzmp-muted"></span></div>
 </form><div id="gzmpAdminSectionsWrap"></div>`;
 const form=$('gzmpAdminVendorStoreForm');
 form.onsubmit=async e=>{e.preventDefault();const f=new FormData(form),b=Object.fromEntries(f.entries()),logo=f.get('logo_file'),banner=f.get('banner_file');delete b.logo_file;delete b.banner_file;try{b.contact_info=JSON.parse(b.contact_info||'{}');b.social_links=JSON.parse(b.social_links||'{}');if(logo?.size)b.logo_url=await uploadAdminImage(logo,'vendor-logo',activeVendor);if(banner?.size)b.banner_url=await uploadAdminImage(banner,'vendor-banner',activeVendor);await api('/api/vendor/admin/vendors/'+encodeURIComponent(activeVendor),{method:'PATCH',body:JSON.stringify(b)});$('gzmpStoreMsg').textContent='Saved successfully.';$('gzmpStoreMsg').style.color='#176b2c';const d=await api('/api/vendor/admin/vendor-data?vendor_id='+encodeURIComponent(activeVendor));vendorData=d;renderVendorStore(d.vendor);await renderVendorSections()}catch(err){$('gzmpStoreMsg').textContent='⚠ '+err.message;$('gzmpStoreMsg').style.color='#a00'}};
};
const renderVendorSettings=(v)=>{
 const pane=$('gzmp-vpane-settings');if(!pane)return;const s=v.vendor_settings||{show_phone:1,show_announcement:1,show_contact:1};
 pane.innerHTML=`<div class="gzmp-list">
 <div class="gzmp-card"><div class="gzmp-toolbar"><div><span class="gzmp-kicker">ACCOUNT</span><h3>Login & security</h3><p class="gzmp-section-sub">Manage the vendor account and login just like the vendor control workflow.</p></div><span class="gzmp-pill ${v.login_exists?'on':'off'}">${v.login_exists?'Login configured':'Login not configured'}</span></div><div class="gzmp-form"><label>Vendor ID<input value="${val(v.id)}" readonly></label><label>Login email<input value="${val(v.login_email||'Not configured')}" readonly></label><label>Role<input value="${val(v.login_role||'vendor_admin')}" readonly></label><label>Status<select id="gzmpSetStatus"><option ${v.status==='Active'?'selected':''}>Active</option><option ${v.status==='Inactive'?'selected':''}>Inactive</option><option ${v.status==='Suspended'?'selected':''}>Suspended</option></select></label></div><div class="gzmp-form" style="margin-top:12px"><label>New password<input id="gzmpSetPassword" type="password" minlength="8"></label><label>Confirm password<input id="gzmpSetPassword2" type="password" minlength="8"></label><div class="full gzmp-actions"><button type="button" class="gzmp-btn primary" id="gzmpSaveAccount">Save account settings</button><span id="gzmpAccountMsg" class="gzmp-muted"></span></div></div></div>
 <div class="gzmp-card"><span class="gzmp-kicker">STOREFRONT PREFERENCES</span><h3>Customer display</h3><p class="gzmp-section-sub">Same switches available in the vendor Settings tab.</p><label class="gzmp-toggle-row"><span>Show phone on storefront</span><input id="gzmpShowPhone" type="checkbox" ${Number(s.show_phone)!==0?'checked':''}></label><label class="gzmp-toggle-row"><span>Show announcement</span><input id="gzmpShowAnnouncement" type="checkbox" ${Number(s.show_announcement)!==0?'checked':''}></label><label class="gzmp-toggle-row"><span>Show contact information</span><input id="gzmpShowContact" type="checkbox" ${Number(s.show_contact)!==0?'checked':''}></label><button type="button" class="gzmp-btn primary" id="gzmpSavePrefs">Save preferences</button><span id="gzmpPrefsMsg" class="gzmp-muted"></span></div>
 <div class="gzmp-card"><span class="gzmp-kicker">ADMIN CONTROLS</span><h3>Marketplace controls</h3><p class="gzmp-section-sub">Shipping, commission, homepage visibility, featured status and vendor activation remain editable from the vendor settings.</p><div class="gzmp-actions"><button type="button" class="gzmp-btn" id="gzmpSettingsStore">Open Store →</button><button type="button" class="gzmp-btn danger" id="gzmpSettingsDelete">Delete account</button></div></div></div>`;
 $('gzmpSaveAccount').onclick=async()=>{const p=$('gzmpSetPassword').value,p2=$('gzmpSetPassword2').value,status=$('gzmpSetStatus').value;if(p&&p!==p2){$('gzmpAccountMsg').textContent='Passwords do not match.';return}try{await api('/api/vendor/admin/vendors/'+encodeURIComponent(activeVendor),{method:'PATCH',body:JSON.stringify({status})});if(p)await api('/api/vendor/admin/reset-password',{method:'POST',body:JSON.stringify({vendor_id:activeVendor,login_email:v.login_email,password:p})});$('gzmpAccountMsg').textContent='Saved successfully.';$('gzmpAccountMsg').style.color='#176b2c'}catch(err){$('gzmpAccountMsg').textContent='⚠ '+err.message;$('gzmpAccountMsg').style.color='#a00'}};
 $('gzmpSavePrefs').onclick=async()=>{try{await api('/api/vendor/admin/vendor-settings?vendor_id='+encodeURIComponent(activeVendor),{method:'PATCH',body:JSON.stringify({show_phone:$('gzmpShowPhone').checked,show_announcement:$('gzmpShowAnnouncement').checked,show_contact:$('gzmpShowContact').checked})});$('gzmpPrefsMsg').textContent='Saved successfully.';$('gzmpPrefsMsg').style.color='#176b2c'}catch(err){$('gzmpPrefsMsg').textContent='⚠ '+err.message;$('gzmpPrefsMsg').style.color='#a00'}};
 $('gzmpSettingsStore').onclick=()=>showVendorSub('store');
 $('gzmpSettingsDelete').onclick=async()=>{if(!confirm('Delete this vendor account? Permanent deletion is allowed only when it has no products or orders.'))return;try{await api('/api/vendor/admin/vendors/'+encodeURIComponent(activeVendor),{method:'DELETE'});closeModal();renderVendors();msg('✓ Vendor account deleted.')}catch(err){msg('⚠ '+err.message,false)}};
};
...
const renderVendorSections=async()=>{
 const d=await api('/api/vendor/admin/store-sections?vendor_id='+encodeURIComponent(activeVendor));const rows=d.sections||[];const pane=$('gzmpAdminSectionsWrap')||$('gzmp-vpane-sections');
