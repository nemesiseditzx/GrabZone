...
const renderVendors=async()=>{
 ...
 box.innerHTML=vs.map(v=>`<div class="gzmp-card gzmp-vendor"><div class="gzmp-vendor-main"><img class="gzmp-avatar" src="${val(v.logo_url||'')}" onerror="this.style.visibility='hidden'"><div style="min-width:0"><div class="gzmp-vendor-name">${val(v.brand_name||v.business_name)}</div><div class="gzmp-muted">${val(v.email)} · ${Number(v.product_count||0)} products · ${Number(v.order_count||0)} orders</div><div style="margin-top:6px"><span class="gzmp-pill ${v.status==='Active'?'on':'off'}">${val(v.status)}</span> <span class="gzmp-pill">${Number(v.homepage_visible)!==0?'Homepage visible':'Hidden'}</span> <span class="gzmp-pill">${Number(v.featured)!==0?'Featured':'Standard'}</span></div></div></div><div class="gzmp-actions"><button class="gzmp-btn primary small" data-manage-vendor="${val(v.id)}">Manage vendor →</button></div></div>`).join('')||'<div class="gzmp-empty">No vendors found.</div>';
...
const renderVendorProducts=products=>{
 ...
 pane.querySelectorAll('[data-edit-product]').forEach(b=>b.onclick=()=>openProductEditor((vendorData.products||[]).find(x=>x.id===b.dataset.editProduct)||{},false));
...
const ensureSecurity=()=>{
 ...
 pane.innerHTML=`<div class="gzmp-card"><h3 class="gzmp-section-title">Vendor login security</h3><p class="gzmp-section-sub">Reset the vendor's login password without needing the current password.</p><form id="gzmpResetPass" class="gzmp-form" style="margin-top:12px"><label>New password<input name="password" type="password" minlength="8" required placeholder="At least 8 characters"></label><label>Confirm password<input name="confirm" type="password" minlength="8" required></label><div class="full gzmp-actions"><button class="gzmp-btn primary">Reset password</button></div><div id="gzmpResetMsg" class="full gzmp-muted"></div></form></div>`;
 $('gzmpResetPass').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.currentTarget),p=f.get('password'),c=f.get('confirm');if(p!==c){$('gzmpResetMsg').textContent='Passwords do not match.';return}try{await api('/api/vendor/admin/reset-password',{method:'POST',body:JSON.stringify({vendor_id:activeVendor,password:p})});$('gzmpResetMsg').textContent='Password reset successfully.';$('gzmpResetMsg').style.color='#176b2c';e.currentTarget.reset()}catch(err){$('gzmpResetMsg').textContent='⚠ '+err.message;$('gzmpResetMsg').style.color='#a00'}};
};
