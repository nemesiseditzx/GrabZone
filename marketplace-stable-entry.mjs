...
const ADMIN_LOADING_SCRIPT = `<style id="gz-admin-loading-style">
...
.gz-load-logo{position:relative;width:116px;height:116px;border-radius:30px;background:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 18px 55px rgba(0,0,0,.10),0 0 0 1px rgba(0,0,0,.04);font-weight:1000;font-size:58px;letter-spacing:-7px;color:#171717;transform:translateZ(0)}
...
<div id="gzAdminLoading" aria-live="polite" aria-label="Loading GrabZone admin panel">
  <div class="gz-load-orbit">
    <div class="gz-load-ring"></div><div class="gz-load-ring two"></div><div class="gz-load-ring three"></div>
    <div class="gz-load-logo">G<span>Z</span></div>
    <div class="gz-load-copy"><div class="gz-load-title">PROCESSING <span class="gz-load-dots">...</span></div><div class="gz-load-sub">PLEASE WAIT A MOMENT</div></div>
    <div class="gz-load-bar"><i></i></div>
  </div>
  <div class="gz-load-corner tl">MORE<br>THAN<br>JUST <b>GRABZONE</b></div>
  <div class="gz-load-corner tr">SHOP<br>EXPLORE<br>ENJOY <b>ONLINE</b></div>
  <div class="gz-load-corner bl">GRABZONE<br>ONLINE STORE <b>GRAB IT. LOVE IT.</b></div>
  <div class="gz-load-corner br">A BETTER<br>SHOPPING<br>EXPERIENCE <b>GZ</b></div>
</div>
<script>
(function(){
  if(window.__gzAdminLoadingInstalled)return;window.__gzAdminLoadingInstalled=true;
  var overlay=document.getElementById('gzAdminLoading'),active=0,timer=0,hideTimer=0,originalFetch=window.fetch;
  function show(){if(!overlay)return;clearTimeout(hideTimer);overlay.classList.remove('gz-hide');}
  function hide(){if(!overlay)return;clearTimeout(hideTimer);hideTimer=setTimeout(function(){if(active===0)overlay.classList.add('gz-hide')},260)}
  window.GZLoading={show:show,hide:hide,start:function(){active++;show()},stop:function(){active=Math.max(0,active-1);hide()}};
  if(originalFetch){window.fetch=function(){var args=arguments,done=false;timer=setTimeout(function(){if(!done){active++;show()}},140);return originalFetch.apply(this,args).then(function(r){done=true;clearTimeout(timer);if(active>0){active--;hide()}return r},function(e){done=true;clearTimeout(timer);if(active>0){active--;hide()}throw e})}}
  ...
  window.addEventListener('pageshow',function(){setTimeout(function(){if(active===0)overlay.classList.add('gz-hide')},350)});
  show();setTimeout(function(){if(active===0)hide()},700);
})();
</script>`;

const ADMIN_PATHS = new Set(['/admin','/admin.html','/vendor-admin','/vendor-admin.html','/marketplace-admin-orders','/marketplace-admin-orders.html','/marketplace-products','/marketplace-products.html','/marketplace-sales','/marketplace-sales.html','/marketplace-stores','/marketplace-stores.html','/marketplace-shipping','/marketplace-shipping.html','/marketplace-settings','/marketplace-settings.html']);

async function addAdminLoader(request, response){
  const path=new URL(request.url).pathname;
  if(!ADMIN_PATHS.has(path) || !response.ok)return response;
  const type=response.headers.get('content-type')||'';
  if(!type.includes('text/html'))return response;
  const html=await response.text();
  if(html.includes('id="gzAdminLoading"'))return new Response(html,response);
  const enhanced=html.replace(/<\/head>/i,ADMIN_LOADING_SCRIPT+'\n</head>');
...
export default {
  async fetch(request, env, ctx) {
    ...
    return addAdminLoader(request,response);
  }
};
