...
async function gzLoadGrowth(){try{const gp=await gzD1({type:"rpc",fn:"rewards_admin_settings",args:{p_action:"get"}});const s=gp.data?.settings||{},x=gzCfgRead(await gzSiteConfig(),GZCFG_MARK,{gpEnabled:1,gpEarnRate:100,gpValue:.1,mysteryEnabled:0,mysteryMin:5,mysteryMax:15});$('growthGpEnabled').value=Number(s.grabpoints_enabled??x.gpEnabled)?1:0;$('growthGpValue').value=Number(s.grabpoints_value??x.gpValue);$('growthMysteryEnabled').value=x.mysteryEnabled;$('growthMysteryMin').value=x.mysteryMin;$('growthMysteryMax').value=x.mysteryMax;$('growthGpStatus').textContent=Number(s.grabpoints_enabled??1)?'● ON':'○ OFF';$('growthMysteryStatus').textContent=x.mysteryEnabled?'● ON':'○ OFF'}catch(e){$('growthGpMsg').textContent='✕ '+e.message}}
async function gzSaveGrowth(){try{const x={gpEnabled:+$('growthGpEnabled').value,gpValue:+$('growthGpValue').value,mysteryEnabled:+$('growthMysteryEnabled').value,mysteryMin:+$('growthMysteryMin').value,mysteryMax:+$('growthMysteryMax').value};if(x.mysteryMin>x.mysteryMax)throw Error('Minimum discount cannot exceed maximum.');await gzD1({type:"rpc",fn:"rewards_admin_settings",args:{p_action:"save",p_grabpoints_enabled:x.gpEnabled,p_earn_rate:100,p_value:x.gpValue,p_tiers:[]}});const css=await gzSiteConfig(),o=gzCfgRead(css,GZCFG_MARK,{gpEnabled:1,gpEarnRate:100,gpValue:.1,mysteryEnabled:0,mysteryMin:5,mysteryMax:15});await gzSaveSiteConfig(GZCFG_MARK,{...o,mysteryEnabled:x.mysteryEnabled,mysteryMin:x.mysteryMin,mysteryMax:x.mysteryMax,gpEnabled:x.gpEnabled,gpValue:x.gpValue,gpEarnRate:100});$('growthGpMsg').textContent='✓ Global GrabPoints settings saved.';gzLoadGrowth()}catch(e){$('growthGpMsg').textContent='✕ '+e.message}}
...
async function gzFillCampaignProducts(id){const r=await gzD1({type:"table",table:"products",action:"select",columns:"id,name,price",orders:[{column:"name",ascending:true}]});const s=$(id);if(!s)return;s.innerHTML='<option value="">Select a product…</option>'+(r.data||[]).map(p=>'<option value="'+esc(p.id)+'">'+esc(p.name)+' — ৳'+Number(p.price).toLocaleString()+'</option>').join('')}async function gzLoadFlashSales(){try{await gzFillCampaignProducts('fsProduct');const r=await gzD1({type:"table",table:"products",action:"select",columns:"id,name,price,flash_price,flash_starts_at,flash_ends_at,flash_enabled",orders:[{column:"name",ascending:true}]});$('flashSaleList').innerHTML=(r.data||[]).filter(p=>p.flash_enabled||p.flash_price).map(p=>'<div class="item" style="margin-top:8px"><b>'+esc(p.name)+'</b><div class="meta">Regular ৳'+Number(p.price).toLocaleString()+' · Flash ৳'+Number(p.flash_price||0).toLocaleString()+' · '+(p.flash_enabled?'Enabled':'Disabled')+' · ends '+(p.flash_ends_at?new Date(p.flash_ends_at).toLocaleString():'—')+'</div><button class="ghost" type="button" onclick="gzDisableFlashSale(\''+p.id+'\')">Disable</button></div>').join('')||'<p class="muted">No flash-sale campaigns yet.</p>'}catch(e){$('flashSaleList').textContent='✕ '+e.message}}async function gzSaveFlashSale(){try{const id=$('fsProduct').value,price=+$('fsPrice').value,start=$('fsStart').value,end=$('fsEnd').value;if(!id||!price||!end)throw Error('Select a product, flash price and end time.');if(start&&Date.parse(start)>=Date.parse(end))throw Error('End time must be after start time.');const p=await gzD1({type:"table",table:"products",action:"select",columns:"price",filters:[{column:"id",op:"eq",value:id}],single:"single"});if(price>=+p.data.price)throw Error('Flash price must be lower than regular price.');await gzD1({type:"table",table:"products",action:"update",filters:[{column:"id",op:"eq",value:id}],values:{flash_price:price,flash_starts_at:start?new Date(start).toISOString():new Date().toISOString(),flash_ends_at:new Date(end).toISOString(),flash_enabled:true}});$('flashSaleMsg').textContent='✓ Flash Sale activated.';gzLoadFlashSales()}catch(e){$('flashSaleMsg').textContent='✕ '+e.message}}async function gzDisableFlashSale(id){try{await gzD1({type:"table",table:"products",action:"update",filters:[{column:"id",op:"eq",value:id}],values:{flash_enabled:false}});gzLoadFlashSales()}catch(e){gzUiToast(e.message,'error')}}async function gzLoadDrops(){try{await gzFillCampaignProducts('dropProduct');const x=gzCfgRead(await gzSiteConfig(),GZDROPS_MARK,{items:[]});$('dropList').innerHTML=(x.items||[]).map((d,i)=>'<div class="item" style="margin-top:8px"><b>'+esc(d.name)+'</b><div class="meta">Starts '+new Date(d.start).toLocaleString()+' · Ends '+new Date(d.end).toLocaleString()+'</div><button class="ghost" onclick="gzDeleteDrop('+i+')">Delete</button></div>').join('')||'<p class="muted">No drops scheduled.</p>'}catch(e){$('dropList').textContent='✕ '+e.message}}
...
async function gzLoadLoyalty(){try{const r=await gzD1({type:"table",table:"customer_points",action:"select",columns:"phone,points,updated_at",orders:[{column:"points",ascending:false}]});$('loyaltyList').innerHTML=(r.data||[]).map(x=>'<div class="item" style="margin-top:8px;display:flex;justify-content:space-between"><b>'+esc(x.phone)+'</b><strong>'+Number(x.points||0).toLocaleString()+' GP</strong></div>').join('')||'<p class="muted">No GrabPoints customers yet.</p>'}catch(e){$('loyaltyList').textContent='✕ '+e.message}}
