...
function renderMain(notices,show){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    const startAnimation=()=>{
      ...
      a.onfinish=()=>{
        if(window.__grabzoneNoticeAnimation!==a)return;
        startAnimation();
      };
    };
...
function renderMarketplace(notices,show){
  ...
  requestAnimationFrame(()=>{
    ...
    moving.style.transform=`translate3d(${width}px,-50%,0)`;
    const a=moving.animate([
      {transform:`translate3d(${width}px,-50%,0)`},
      {transform:`translate3d(${-content-width}px,-50%,0)`}
    ],{duration:Math.max(12000,((width+content)/150)*1000),iterations:Infinity,easing:'linear'});
    animation=a;
  });
  return true;
}
