...
const KEY='grabzone_cart_v2',BUY='grabzone_buy_now_v2',MODE='grabzone_checkout_mode_v1',currency=C.currency||'৳';
const $=id=>document.getElementById(id);
const esc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const normalizeItem=x=>{const q=Math.max(1,Math.floor(Number(x?.quantity||1)));const unit=Number(x?.unit_price??x?.price??0);return {...x,product_id:String(x?.product_id||x?.id||''),unit_price:Number.isFinite(unit)?unit:0,price:Number.isFinite(unit)?unit:0,quantity:q}};
const read=()=>{try{const v=JSON.parse(localStorage.getItem(KEY)||'[]');return Array.isArray(v)?v.filter(x=>x&&String(x.product_id||x.id||'')).map(normalizeItem):[]}catch{return[]}};
const write=v=>localStorage.setItem(KEY,JSON.stringify(v.map(normalizeItem)));
...
function add(item,qty=1){cart=read();const normalized=normalizeItem(item),found=cart.find(x=>cartKey(x)===cartKey(normalized));if(found)found.quantity+=Math.max(1,Math.floor(Number(qty)||1));else cart.push({...normalized,quantity:Math.max(1,Math.floor(Number(qty)||1))});write(cart);update();openDrawer()}
...
function checkout(items,buyNow=true){if(buyNow){localStorage.setItem(BUY,JSON.stringify(items.map(normalizeItem)));localStorage.setItem(MODE,'buy')}else{localStorage.removeItem(BUY);localStorage.setItem(MODE,'cart')}location.href='checkout.html'}
...
async function variationAction(product,action){
  ...
  const actualProduct=d.product||product;const vs=(d.variations||[]).filter(v=>v.status!=='Disabled');
  if(!vs.length){if(String(actualProduct.product_type||product.product_type||'').toLowerCase()==='variable'){alert('This variable product has no available variations yet. Please contact the store.');return}return action({...product,quantity:1)}
  ...
  const update=()=>{chosen=vs.find(v=>names.every(n=>selected[n]&&v.options?.[n]===selected[n]))||null;state.textContent=chosen?(chosen.status==='Available'?'✓ '+Object.entries(chosen.options||{}).map(([k,v])=>k+': '+v).join(' · ')+' · '+money(Number(chosen.sale_price>0&&chosen.sale_price<chosen.regular_price?chosen.sale_price:chosen.regular_price)||0):(chosen.status||'Unavailable')):'Select all options';go.disabled=!chosen||chosen.status!=='Available';modal.querySelectorAll('[data-n]').forEach(b=>b.classList.toggle('active',selected[b.dataset.n]===b.dataset.v))};
