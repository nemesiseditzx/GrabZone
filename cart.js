...
async function chooseVariant(p,qty=1,mode='add'){
  ...
  try{
    ...
    go.onclick=()=>{if(!current)return;const n=Math.max(1,Number(qty||1)),stock=stockOf(current);if(!isAvailable(current)){alert('This variation is unavailable.');return}const price=Number(current.regular_price||0);const item={product_id:p.id,variation_id:current.id,variation_options:current.options||{},name:p.name||'Product',image_url:current.image_url||p.image_url||'',price,unit_price:price,sku:current.sku||'',quantity:n};close();mode==='buy'?checkout([item]):add(item,n)};
    refresh();return true;
