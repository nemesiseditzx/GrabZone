...
async function chooseVariant(p,qty,mode){
  ...
  if(!vars.length){
    if(!stockAvailable(p,null,qty)){alert('Not enough stock for '+p.name+'.');return}
    const item={product_id:p.id,name:p.name,image_url:p.image_url,price:Number(p.price),unit_price:Number(p.price),quantity:qty};
    if(mode==='buy')return checkout([item]);return add(item,qty)
  }
