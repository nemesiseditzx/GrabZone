...
async function chooseVariant(p,qty=1,mode='add'){
  ...
  try{
    ...
    go.onclick=()=>{if(!current)return;const n=Math.max(1,Number(qty||1)),stock=stockOf(current);if(current.status!=='Available'||stock<n){alert('This variation is out of stock.');return}const regular=Number(current.regular_price||0),sale=Number(current.sale_price||0),price=sale>0&&sale<regular?sale:regular;const item={product_id:p.id,variation_id:current.id,variation_options:current.options||{},variation_stock_managed:true,name:p.name||'Product',image_url:current.image_url||p.image_url||'',price,unit_price:price,sku:current.sku||'',quantity:n};close();mode==='buy'?checkout([item]):add(item,n)};
    refresh();return true;
