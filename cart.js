...
async function productData(id){const cached=Array.isArray(window.__grabzoneProducts)?window.__grabzoneProducts.find(x=>String(x.id)===String(id)):null;if(cached)return cached;if(!sb)return null;const{data}=await sb.from('products').select('id,name,price,image_url,published,stock').eq('id',id).maybeSingle();return data||null})();