...
async function chooseVariant(p,qty=1,mode='add'){
  ...
  try{
    ...
    modal.innerHTML='<div class="gz-vp-backdrop"></div><section class="gz-vp-card" role="dialog" aria-modal="true"><button type="button" class="gz-vp-close">×</button><h3>'+esc(p.name||'Choose options')+'</h3><p class="gz-vp-sub">Select all options</p><div class="gz-vp-options"></div><div class="gz-vp-state">Select all options</div><div class="gz-vp-actions"><button type="button" class="gz-vp-cancel">Cancel</button><button type="button" class="gz-vp-go">'+(mode==='buy'?'Buy Now':'Add to Cart')+'</button></div></section></div>';
    ...
    const isAvailable=(v,n=1)=>!!v&&v.status==='Available'&&(v.stock_mode!=='tracked'||stockOf(v)>=n);
    const refresh=()=>{current=match();area.querySelectorAll('button[data-n]').forEach(b=>{const on=selected[b.dataset.n]===b.dataset.v;b.classList.toggle('selected',on);b.classList.toggle('unavailable',!on&&!compatible(b.dataset.n,b.dataset.v))});const n=Math.max(1,Number(qty||1)),stock=stockOf(current);if(!current){state.textContent='Select all options';state.className='gz-vp-state';go.disabled=true;return}const available=isAvailable(current,n);state.textContent=available?(current.stock_mode==='tracked'?'✓ In Stock · '+stock+' available':'✓ In Stock'):'Out of Stock';state.className='gz-vp-state'+(available?' ok':'');go.disabled=!available};
    ...
    go.onclick=()=>{if(!current)return;const n=Math.max(1,Number(qty||1)),stock=stockOf(current);if(!isAvailable(current,n)){alert('This variation is out of stock.');return}const regular=Number(current.regular_price||0),sale=Number(current.sale_price||0),price=sale>0&&sale<regular?sale:regular;const item={product_id:p.id,variation_id:current.id,variation_options:current.options||{},variation_stock_managed:true,name:p.name||'Product',image_url:current.image_url||p.image_url||'',price,unit_price:price,sku:current.sku||'',quantity:n};close();mode==='buy'?checkout([item]):add(item,n)};
    refresh();return true;
