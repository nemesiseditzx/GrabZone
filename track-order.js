...
(()=> {
 ...
 const shipmentMarkup=vendors.length?vendors.map(v=>{
   ...
   const invClose=$('invoiceClose');if(invClose)invClose.onclick=()=>$('invoiceModal').classList.remove('open');const invModal=$('invoiceModal');if(invModal)invModal.onclick=e=>{if(e.target===invModal)invModal.classList.remove('open')};
 }
