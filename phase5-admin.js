...
async function reviewAction(id,action){
 try{const t=window.getToken?window.getToken():'';if(action==='delete'){if(!await gzUiConfirm('Delete this review permanently?'))return;let r=await fetch(BACKEND+'/api/product-reviews?id='+encodeURIComponent(id),{method:'DELETE',headers:{Authorization:'Bearer '+t}});if(!r.ok)throw Error((await r.json()).catch(()=>({})).error||'Delete failed')}else{let row=document.querySelector('[data-id="'+CSS.escape(id)+'"]');let approved=row?.textContent.includes('Hide');let r=await fetch(BACKEND+'/api/product-reviews',{method:'PATCH',headers:{Authorization:'Bearer '+t,'Content-Type':'application/json'},body:JSON.stringify({id,approved:!approved})});if(!r.ok)throw Error('Update failed')}reviews()}catch(e){gzUiToast(e.message,'error')}
}
function inject(){
