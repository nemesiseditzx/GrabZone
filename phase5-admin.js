...
async function reviews(){
 ...
 try{
  const t=window.getToken?window.getToken():'';let r=await (window.gzAuthFetch||fetch)(BACKEND+'/api/product-reviews?product_id='+encodeURIComponent($('p5_product').value)+'&admin=1',{headers:{Authorization:'Bearer '+t},credentials:'include'});let d=await r.json().catch(()=>({}));if(!r.ok)throw Error(d.error||'Could not load reviews');
...
async function reviewAction(id,action){
 try{const t=window.getToken?window.getToken():'';if(action==='delete'){if(!await gzUiConfirm('Delete this review permanently?'))return;let r=await (window.gzAuthFetch||fetch)(BACKEND+'/api/product-reviews?id='+encodeURIComponent(id),{method:'DELETE',headers:{Authorization:'Bearer '+t},credentials:'include'});if(!r.ok){const er=await r.json().catch(()=>({}));throw Error(er.error||'Delete failed')}}else{let row=document.querySelector('[data-id="'+CSS.escape(id)+'"]');let approved=row?.textContent.includes('Hide');let r=await (window.gzAuthFetch||fetch)(BACKEND+'/api/product-reviews',{method:'PATCH',headers:{Authorization:'Bearer '+t,'Content-Type':'application/json'},credentials:'include',body:JSON.stringify({id,approved:!approved})});if(!r.ok)throw Error('Update failed')}reviews()}catch(e){gzUiToast(e.message,'error')}
}
function inject(){
