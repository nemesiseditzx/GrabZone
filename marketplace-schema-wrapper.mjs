...
async function migrate(env){
  ...
  const migrations=[
    ...
    ['products','vendor_id','TEXT'],['products','stock','INTEGER DEFAULT 0'],['products','sku','TEXT'],['products','category_id','TEXT'],['order_items','vendor_id','TEXT'],
...
async function createVendor(request,env,ctx){
  const p=new URL(request.url).pathname;if(request.method!=='POST'||p!=='/api/vendor/admin/vendors')return null;
  ...
  const existing=await env.DB.prepare('SELECT vu.id,vu.vendor_id,v.brand_name,v.slug,v.status FROM vendor_users vu JOIN vendors v ON v.id=vu.vendor_id WHERE lower(vu.email)=lower(?) LIMIT 1').bind(email).all();
  if(existing.results?.length){const v=existing.results[0];return json({error:`This login email is already assigned to vendor "${v.brand_name||v.slug}". Use a different vendor login email.`},409)}
  ...
  try{
    const k=await crypto.subtle.importKey('raw',new TextEncoder().encode(pass),'PBKDF2',false,['deriveBits']);const bits=await crypto.subtle.deriveBits({name:'PBKDF2',salt:new TextEncoder().encode(salt),iterations:100000,hash:'SHA-256'},k,256);const passwordHash=[...new Uint8Array(bits)].map(x=>x.toString(16).padStart(2,'0')).join('');
    await env.DB.batch([
...
async function saveVendorPatch(request,env,ctx){const p=new URL(request.url).pathname;if(request.method!=='PATCH'||!p.startsWith('/api/vendor/admin/vendors/'))return null;if(!await adminOK(request,env,ctx))return json({error:'Unauthorized'},401);const id=clean(p.split('/').pop(),100),b=await request.clone().json().catch(()=>null);if(!b||typeof b!=='object')return json({error:'Invalid JSON'},400);const fields=['name','business_name','brand_name','email','phone','logo_url','banner_url','description','tagline','accent_color','status','shipping_fee','commission_type','commission_value','homepage_visible','featured','announcement','social_links','contact_info','business_email','order_notification_email','support_email'];const sets=[],params=[];for(const k of fields)if(b[k]!==undefined){let v=['social_links','contact_info'].includes(k)?JSON.stringify(b[k]||{}):['shipping_fee','commission_value'].includes(k)?Math.max(0,Number(b[k])):['homepage_visible','featured'].includes(k)?(b[k]?1:0):clean(b[k],10000);sets.push(k+'=?');params.push(v)}if(!sets.length)return json({ok:true});sets.push('updated_at=?');params.push(now());try{await env.DB.prepare('UPDATE vendors SET '+sets.join(',')+' WHERE id=? OR slug=?').bind(...params,id,id).run();return json({ok:true})}catch(err){return json({error:err?.message||'Failed to save vendor'},500)}}

async function vendorSession(req,env){const raw=cookie(req,'gz_vendor_session');if(!raw)return null;return (await env.DB.prepare("SELECT vu.id,vu.vendor_id,vu.email,vu.role,v.status vendor_status FROM vendor_sessions s JOIN vendor_users vu ON vu.id=s.vendor_user_id JOIN vendors v ON v.id=vu.vendor_id WHERE s.token_hash=? AND s.expires_at>? AND vu.status='Active' AND v.status='Active'").bind(await sha(raw),now()).all()).results?.[0]||null}

async function protectShipment(request,env,ctx){const p=new URL(request.url).pathname;if(request.method!=='POST'||p!=='/api/vendor/shipments')return null;const u=await vendorSession(request,env);if(!u)return json({error:'Unauthorized'},401);const b=await request.clone().json().catch(()=>({})),vendorOrderId=clean(b.vendor_order_id,100);const order=(await env.DB.prepare('SELECT vo.*,o.order_number FROM vendor_orders vo JOIN orders o ON o.id=vo.order_id WHERE vo.id=? AND vo.vendor_id=?').bind(vendorOrderId,u.vendor_id).all()).results?.[0];if(!order)return json({error:'Vendor order not found'},404);const items=(await env.DB.prepare('SELECT id,order_item_id,product_id,quantity FROM vendor_order_items WHERE vendor_order_id=?').bind(order.id).all()).results||[];if(!items.length)return json({error:'This vendor order has no shippable items.'},409);const remaining=[];for(const item of items){const shipped=Number((await env.DB.prepare('SELECT COALESCE(SUM(si.quantity),0) n FROM shipment_items si JOIN shipments s ON s.id=si.shipment_id WHERE s.order_id=? AND s.vendor_id=? AND si.order_item_id=?').bind(order.order_id,u.vendor_id,item.order_item_id).all()).results?.[0]?.n||0);const ordered=Math.max(0,Number(item.quantity||0));if(shipped>ordered)return json({error:'Shipment data is inconsistent for one or more items.'},409);if(ordered-shipped>0)remaining.push({order_item_id:item.order_item_id,quantity:ordered-shipped})}if(!remaining.length)return json({error:'All items in this vendor order have already been shipped.'},409);const r=await app.fetch(request,env,ctx);if(!r.ok)return r;const d=await r.clone().json().catch(()=>null),shipmentId=d?.shipment_id;if(shipmentId){try{await env.DB.batch(remaining.map(x=>env.DB.prepare('INSERT INTO shipment_items(id,shipment_id,order_item_id,quantity) VALUES(?,?,?,?)').bind(crypto.randomUUID(),shipmentId,x.order_item_id,x.quantity)))}catch(err){await env.DB.prepare('DELETE FROM shipments WHERE id=? AND vendor_id=?').bind(shipmentId,u.vendor_id).run().catch(()=>{});return json({error:'Shipment item mapping failed. Shipment was rolled back.'},500)}}return r}

function hardenResponse(request,response){const h=new Headers(response.headers);h.delete('Access-Control-Allow-Origin');h.delete('Access-Control-Allow-Credentials');h.delete('Access-Control-Allow-Headers');h.delete('Access-Control-Allow-Methods');return new Response(response.body,{status:response.status,statusText:response.statusText,headers:h})}
