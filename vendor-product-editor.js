(()=>{
  ...
  function handleFiles(){const files=[...($('#vpFiles').files||[])];if(files.length>MAX){window.gzNotify('Maximum 10 images per product.');$('#vpFiles').value='';state.files=[];renderMedia();return}const bad=files.find(f=>f.size>MB);if(bad){window.gzNotify(`${bad.name} is larger than 1 MB. Maximum is 1 MB per image.`);$('#vpFiles').value='';state.files=[];renderMedia();return}state.files=files;state.mainIndex=0;renderMedia()}
