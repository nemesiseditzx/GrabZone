...
export default{async fetch(req,env,ctx){try{const a=await normalizeAdmin(req,env),r=await normalizeVendor(a,env),p=new URL(r.url).pathname;const category=await marketplaceCategories(r,env);if(category)return category;
