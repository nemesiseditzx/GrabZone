(()=>{
 ...
 form.addEventListener('submit',async e=>{
   ...
   try{
     ...
     result.innerHTML=`<b>Order #${esc(o.order_number||id)}</b><div class="current-status">Overall Status: ${esc(o.status||'Processing')}<small>Tracking ID: ${esc(o.tracking_id||id)}</small></div><div class="tracking-box"><b>Order Summary</b><div style="margin-top:7px">Subtotal: ${money(o.subtotal)} · Shipping: ${money(o.shipping_charge)} · Total: ${money(o.total)}</div></div>${vendors.map(v=>`<section class="tracking-box" style="margin-top:14px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:center"><div><b>${esc(v.brand_name)}</b><div style="font-size:12px;color:#777;margin-top:3px">${esc(v.status||'Processing')}</div></div></div><div class="items">${(v.items||[]).map(i=>`<div class="item"><span>${esc(i.product_name)} × ${i.quantity}</span><b>${money(i.line_total)}</b></div>`).join('')||'<div style="color:#777">No items mapped.</div>'}</div>${(v.shipments||[]).map(s=>`<div style="margin-top:10px;padding:12px;background:#fff;border:1px solid #e4e4df;border-radius:12px"><div style="font-size:12px;color:#555;margin-bottom:6px"><b>Shop: ${esc(v.brand_name||v.vendor_name||'Vendor')}</b><br><span>Product: ${esc((v.items||[]).map(i=>i.product_name+' × '+i.quantity).join(' · ')||'Product')}</span></div><b>${esc(s.status||'Processing')}</b><div style="margin-top:4px">${esc(s.courier||'Courier pending')} · ${esc(s.tracking_id||'Tracking pending')}</div>${s.tracking_url?`<a class="tracking-link" href="${esc(s.tracking_url)}" target="_blank" rel="noopener">Track Package →</a>`:''}</div>`).join('')||'<div style="margin-top:10px;color:#777">Shipment not added yet.</div>'}</section>`).join('')}`;
